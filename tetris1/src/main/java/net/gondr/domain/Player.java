...
public class Player {
	...
	private int next2 = 0; //다다음 모양
	private int nextColor2 = 0; //다다음 색상
	...
	public Player(Block[][] board) {
		...
		next2 = rnd.nextInt(shape.length); //다다음 블럭 뽑고
		nextColor2 = next2;
	...
	private void getNextBlock() {
		...
		next2 = rnd.nextInt(shape.length);
		nextColor2 = next2;
...
//	}
	...
	public void render(GraphicsContext gc, GraphicsContext gc2 ,double width, double height) {
		Color color = colorSet[nextColor];
		Color color2 = colorSet[nextColor2];
		Point2D[] block = shape[next][0];
		Point2D[] block2 = shape[next2][0];
		...
		for(int i = 0; i < block2.length; i++) {
			double dx = x + block2[i].getX() * size;
			double dy = y + block2[i].getY() * size;
