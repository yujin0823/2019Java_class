...
public class Game {
	...
	private AnimationTimer mainLoop; //게임 루프, 쓰레드
	...
	private Label scoreLabel; //점수판
	
	private Canvas nextCanvas; //다음 블록 가져오기
	private Canvas next2Canvas; //다다음 블로가겾오기
	
	private double nbWidth; //nextCanvas의 너비 높이
	...
	public Game(Canvas canvas, Canvas nextCanvas, 
			Label scoreLabel, ObservableList<ScoreVO> list, Canvas next2Canvas) {
		this.nextCanvas = nextCanvas;
		this.next2Canvas = next2Canvas;
		...
		//다다음 블럭 그려줄 캔버스를 설정
		this.nbWidth = this.next2Canvas.getWidth();
		this.nbHeight = this.next2Canvas.getHeight();
		this.nbGC2 = this.next2Canvas.getGraphicsContext2D();
	...
	public void update(double delta) {
		if(gameOver) return; //만약 gameOver가 true이면 아무것도 ㄴㄴ
		blockDownTime += delta;		
	...
	public void render() {
		...
		player.render(nbGC, nbGC2 ,nbWidth, nbHeight);
	}
